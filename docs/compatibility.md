...
- **Folder notes** — the best pairing. Giving every folder a note of its own makes each breadcrumb segment a real destination, reachable two ways: the folder's note appears in that folder's dropdown, and the **underlined delimiter after each folder opens it on click**. Moving or renaming a folder note in rename mode works like any other file.
  ...
  With **Folder notes** running, the note is resolved here under that plugin's own convention and opened directly; re-dispatching the click onto Obsidian's own breadcrumb element is the fallback for a folder with no note, so whatever owns that element still gets its say. Only that plugin publishes a convention to read, so with either of the others running the press stays Obsidian's own and reveals the folder, exactly as it always did. Delegation alone used to be the whole of it, which worked only as far as the peer's own marking reached — see the underline below.
  ...
  The underline is marked here for the same reason. It used to key purely off the `has-folder-note` class that **Folder notes** (LostPaul) puts on the native path segment — and that plugin marks only the segments it recognises, which in practice means top-level folders: on `parallel structures/parent1/childa/unnname` nothing was underlined and the press did nothing either, with the note sitting right there. (Not this plugin's doing: with Lure disabled that press is just as inert.) With that plugin running, Lure marks a segment whose folder really has a note, resolved through its own `folderNoteName`, `folderNoteType` and `storageLocation`, and still honours the peer's class where it appears — so the underline and the press agree at every depth. With the other two, which publish no convention and claim nothing, no delimiter is underlined and none opens a note: the same behaviour as before, and the same as with no folder-note plugin at all.
