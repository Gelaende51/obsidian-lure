...
- **A page can be typed, not only picked.** `:graph`, `:search`, or whatever your plugins register — the labels the [vault root's listing](#a-pane-with-no-file) offers. Typing a colon anywhere summons them, since no name may contain one, and <kbd>Enter</kbd> opens that view in this pane. <kbd>Tab</kbd> finishes the name as it finishes a folder's, and clicking the label on such a pane opens the field already holding it.
