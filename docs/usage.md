...
- **A page can be typed, not only picked.** `:graph`, `:search`, or whatever your plugins register — the labels the [vault root's listing](#a-pane-with-no-file) offers. Typing a colon anywhere summons them, since no name may contain one, and <kbd>Enter</kbd> opens that view in this pane.
...
| **Show dot files** | On / Off | Off | Whether dot-files and dot-folders are listed in the dropdowns. Overwrite protection applies either way. The row carries a link to Obsidian's own *Show all file types*, which is the other rule deciding what a dropdown can list — inside the vault; outside it, that setting does not apply. |
