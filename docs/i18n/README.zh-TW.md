...
- **用量** — 2026 年 8 月 3–22 日，22 次工作階段，約 11,820 次回覆：產生約 1540 萬個詞元，送出約 5550 萬，快取重讀約 36.442 億（合計約 37.15 億）。
- **上游** — 模型是從他人公開的開放原始碼、文件與社群文字中學會的。大部分功勞屬於他們。
...
- **清單會在你所在的項目上開啟**，以方向鍵或指標瀏覽時，會把你指向的內容填入輸入框。越過任一端時，它會把你原本輸入的內容還給你。
- **名稱會隨著輸入自動補全** — 在資料夾裡的名稱一致之處，一致的部分會出現在游標之後並被選取；<kbd>Tab</kbd> 或 <kbd>→</kbd> 整段接受，<kbd>Backspace</kbd> 退回。
- **<kbd>Tab</kbd> 像 shell 一樣補全**：把你輸入的內容延伸到名稱一致為止，只剩一個名稱時便走進那個資料夾。越過路徑末端則改為擴大選取範圍：名稱、含副檔名的名稱、自儲存庫起的路徑、自系統根目錄起的路徑。<kbd>Shift</kbd>+<kbd>Tab</kbd> 沿同一條路倒著走。
- **右鍵複製** — 兩次取名稱，三次取它右側的一切，在空白處則取整條路徑或系統路徑。
- **把一則筆記拖到路徑列的某個資料夾上**，即可連同連結一起移過去。儲存庫名稱也接受，用於根目錄；整片選取會作為一個整體移動，接不下的資料夾則什麼也不顯示。
- **輸入網址** — `https://`、`obsidian://`，或 `file://` 及百分比編碼的路徑 — 便會被開啟，而不是當作筆記名稱。
- **長路徑會從字母冗餘之處縮短** — 絕不越過區分相鄰資料夾所需的部分 — 只有再無可壓縮時才捲動。把指標停在縮短的名稱上，即可看回完整的它。
...
- **設定：**對齊方式、分隔符號預設值、哪一次點擊開啟清單、儲存庫名稱、點檔案、副檔名。
