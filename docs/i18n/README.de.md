...
- **Die Vault-Wurzel listet die Seiten, die ein Pane halten kann** — `:graph`, `:search` und was auch immer deine Plugins registrieren. Wähle eine aus, oder tippe sie: Kein Dateiname beginnt mit einem Doppelpunkt, also dienen die Bezeichnungen zugleich als Adresse. Ist ein Startseiten-Plugin installiert, öffnet das eigene Trennzeichen des Vaults diese Seite beim ersten Klick und klappt beim nächsten den Dateibaum weg.
