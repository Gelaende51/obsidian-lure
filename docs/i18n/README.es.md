<!-- Traducción de README.md — estado: commit 2cbb237.
...
- **<kbd>F2</kbd>** alterna entre el título en línea y la barra de ruta, abriéndose sobre el nombre sin su extensión y ampliándose hasta las rutas completas con más pulsaciones. Atraviesa limpiamente el diálogo de renombrar de Obsidian cuando el título ha quedado fuera de la vista. Un comando *Enfocar la barra de ruta* recorre los mismos peldaños sin renombrar; la fila de *Atajos de teclado* en los ajustes te lleva a asignarle uno.
...
- **La raíz de la bóveda lista las páginas que puede contener un panel** — `:graph`, `:search`, y cualquier vista que registren tus plugins. Elige una, o escríbela: ningún nombre de archivo empieza con dos puntos, así que las etiquetas también funcionan como dirección. `:graph` escrito dentro de una carpeta abre el grafo de esa carpeta. Con un plugin de página de inicio instalado, el separador propio de la bóveda abre esa página al primer clic y pliega el árbol de archivos al siguiente.
- **Una fila en los paneles que no contienen ningún archivo** — una pestaña vacía se lee `vault / :blank`, el grafo `vault / :graph`, y el campo de al lado es una barra de direcciones: escribe una ruta y <kbd>Intro</kbd> la abre en ese panel, o la crea. Los paneles de la barra lateral conservan el título propio de Obsidian.
...
*En modo mover/renombrar el mismo desplegable ofrece otra cosa: el nombre actual de la nota fijado arriba para moverla sin renombrarla, carpetas a las que llevarla, y los nombres ya ocupados en rojo; elegir uno pregunta qué hacer con el archivo en el camino.*
