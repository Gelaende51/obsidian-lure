<!-- Traducción de CHANGELOG.md — estado: commit 2739cf0.
...
- **Un nombre ya utilizado pregunta en lugar de rechazar.** Mover o renombrar a un nombre que ya existe abre un diálogo con dos rutas que puedes editar: adónde va tu archivo, y adónde va el archivo que estorba, en rojo mientras siga ocupada. Cada ruta se dibuja también como la dibuja la barra de ruta, con las partes que difieren coloreadas y acortadas en último lugar. Ambos campos tienen una lista; la segunda contiene las salidas habituales — intercambiar lugares (va a la antigua carpeta de tu archivo), intercambiar nombres (se queda donde está y toma el nombre antiguo de tu archivo), intercambiar ambos (toma la ruta antigua de tu archivo), `-1`, `-bak` y `-old` junto a su propio nombre, y los dos nombres que tenían los archivos. Una salida cuya ruta está ocupada aparece en gris. Elegir una solo rellena el campo; Aplicar mueve ambos, enlaces incluidos, y Cancelar no mueve nada. Elegir un nombre ya utilizado en el desplegable pregunta lo mismo, igual que soltar un archivo sobre una carpeta que ya contiene su nombre.
...
- **La propuesta es siempre lo que Tab escribiría.** Donde los nombres dejan de coincidir, el campo propone el paso hacia el primero de ellos, y la fila hacia la que Tab se dirigiría es la que decide; escribir sobre un nombre deja su extensión donde estaba y la propone delante de ella; una carpeta en la que acabas de entrar propone su primer paso. Antes había estados en los que no se proponía nada, y aun así Tab escribía algo. El subrayado del desplegable sigue a la propuesta mientras cambia, y Tab en una fila a la que has llegado con las flechas toma esa fila en vez de la de al lado.
...
- **Los nombres que empiezan por lo que escribiste van primero en el desplegable**, marcados con una línea en el borde — azul donde comparten más de lo que escribiste, verde en la rama que toma la propuesta donde se separan — por delante de los nombres que solo lo contienen. Cada uno subraya el paso que <kbd>Tab</kbd> daría hacia él, no solo el que se propone.
...
- **Retroceso ante una extensión que ha quedado sola sube a la carpeta superior**, igual que en un campo vacío; la extensión solitaria desaparece.
