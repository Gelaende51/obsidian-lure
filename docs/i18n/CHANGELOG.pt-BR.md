<!-- Tradução de CHANGELOG.md — estado: commit 2739cf0.
...
- **Um nome já em uso pergunta em vez de recusar.** Mover ou renomear para um nome que já existe ali abre uma caixa de diálogo com dois caminhos que você pode editar: para onde seu arquivo vai, e para onde vai o arquivo que está no caminho, em vermelho enquanto isso ainda for verdade. Cada caminho também é desenhado do jeito que a barra de caminho desenha um, com as partes que diferem coloridas e encurtadas por último. Os dois campos têm uma lista; a do segundo traz as saídas de sempre — trocar de lugares (ele vai para a pasta antiga do seu arquivo), trocar nomes (ele fica onde está e fica com o nome antigo do seu arquivo), trocar tudo (ele fica com o caminho antigo do seu arquivo), `-1`, `-bak` e `-old` ao lado do próprio nome, e os dois nomes que os arquivos tinham. Uma saída cujo caminho já está em uso fica acinzentada. Escolher uma só preenche o campo; Aplicar move os dois, com os links e tudo, e Cancelar não move nada. Escolher um nome já em uso na lista pergunta a mesma coisa, assim como arrastar um arquivo para uma pasta que já tenha esse nome.
...
- **Nomes que começam com o que você digitou vêm primeiro na lista**, marcados com uma linha na borda — azul onde compartilham mais do que você digitou, verde no ramo que a sugestão segue onde eles se separam — antes dos nomes que só o contêm. Cada um deles sublinha o passo que <kbd>Tab</kbd> daria em direção a ele, não só o que está sendo sugerido.
...
[^1.5.0]: Alterações desde a 1.4.0: <https://github.com/Gelaende51/obsidian-lure/compare/1.4.0...1.5.0>
