...
- **Navn fullfører seg selv mens du skriver** — det <kbd>Tab</kbd> ville skrevet, vises etter markøren, merket — overensstemmelsen mellom navnene i mappen, eller steget mot det første av dem; skriving sluker den bokstav for bokstav, <kbd>→</kbd> tar én bokstav, <kbd>Tab</kbd> eller <kbd>End</kbd> tar den hel, <kbd>Rettetast</kbd> tar den tilbake. Listen fortsetter å filtrere etter det du skrev, ikke etter det som ble tilbudt.
