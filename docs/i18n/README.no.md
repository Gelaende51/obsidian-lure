<!-- Oversettelse av README.md — status: commit 99fc83e.
...
- **Listen åpner på oppføringen du står i**, og å gå gjennom den med piltastene eller pekeren fyller feltet med det du peker på. En rad du peker på, vises som tilbudet den ville gitt; forbi begge ender av listen får du tilbake det du hadde skrevet, og tar du pekeren bort fra den, går merkingen tilbake dit du var. Listen følger markøren: mappen den er i, filtrert etter bokstavene foran den.
- **Blyantknappen på mappe** stiller de samme handlingene om til å gi nytt navn / flytte, med de samme kontrollene Obsidian selv gjør. Et navn som allerede er tatt, er rødt i listen, og å velge det spør om du vil gi nytt navn til det som er i veien, eller bytte plass eller navn med det.
...
- **Navn fullfører seg selv mens du skriver** — det <kbd>Tab</kbd> ville skrevet, vises etter markøren, merket og stavet slik navnet er, uansett store eller små bokstaver du skrev — overensstemmelsen mellom navnene i mappen, eller steget mot det første av dem; skriving sluker den bokstav for bokstav, <kbd>→</kbd> tar én bokstav, <kbd>Tab</kbd> eller <kbd>End</kbd> tar den hel, <kbd>Rettetast</kbd> tar den tilbake. Listen fortsetter å filtrere etter det du skrev, ikke etter det som ble tilbudt.
...
- **Hvelvets rot lister opp sidene et panel kan holde** — `:graph`, `:search` og alle visninger tilleggene dine registrerer. Velg en, eller skriv den inn: et kolon begynner ingen filnavn, så etikettene fungerer også som adresse. `:graph` skrevet inne i en mappe åpner grafen til den mappen. Med et tillegg for startside installert åpner hvelvets eget skilletegn den siden ved første klikk og bretter filtreet sammen ved det neste.
...
*I gi nytt navn-/flyttemodus tilbyr den samme listen noe annet: notatets nåværende navn festet øverst, så det kan flyttes uten å få nytt navn; mapper å flytte det inn i; og navn som allerede er tatt i rødt; velger du ett, blir du spurt hva som skal skje med filen som står i veien.*
