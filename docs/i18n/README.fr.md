...
- **La racine du coffre liste les pages qu'un volet peut contenir** — `:graph`, `:search`, et toutes les vues que vos plugins enregistrent. Choisissez-en une, ou tapez-la : un deux-points ne commence jamais un nom de fichier, si bien que les étiquettes font aussi office d'adresse. Avec un plugin de page de démarrage installé, le séparateur du coffre ouvre cette page au premier clic et replie l'arborescence de fichiers au suivant.
