<!-- Traduction de README.md — état : commit 2cbb237.
...
- **Le menu s'ouvre sur l'entrée où vous vous trouvez**, et le parcourir aux flèches ou au pointeur remplit le champ avec ce que vous visez. Une ligne que vous pointez s'affiche comme la proposition qu'elle ferait ; sortir par l'une ou l'autre extrémité de la liste vous rend ce que vous aviez tapé, et retirer le pointeur de la liste ramène la surbrillance là où vous étiez. La liste suit le curseur : le dossier où il se trouve, filtré par les lettres qui le précèdent.
- **Le bouton crayon-dossier** bascule les mêmes interactions en mode déplacer/renommer, avec les mêmes validations qu'Obsidian applique. Un nom déjà pris apparaît en rouge dans la liste, et le choisir ouvre les deux chemins côte à côte à modifier, avec l'échange des emplacements, des noms ou des deux à portée d'un choix.
...
- **Les noms se complètent à la frappe** — ce que <kbd>Tab</kbd> écrirait apparaît après le curseur, sélectionné et orthographié comme le nom l'est, quelle que soit la casse que vous avez tapée — la concordance des noms du dossier, ou le pas vers le premier d'entre eux ; la frappe l'absorbe lettre après lettre, <kbd>→</kbd> en prend une lettre, <kbd>Tab</kbd> ou <kbd>Fin</kbd> la prend entière, <kbd>Retour arrière</kbd> la reprend. Le menu continue de filtrer selon ce que vous avez tapé, pas selon ce qui vous a été proposé.
...
- **Saisissez une URL** — `https://`, `obsidian://`, ou un chemin `file://` ou encodé en pourcentage — et elle est ouverte au lieu d'être prise pour un nom de note. Les adresses web s'ouvrent dans un onglet de l'*Afficheur web* d'Obsidian si vous l'avez activé.
...
- **La racine du coffre liste les pages qu'un volet peut contenir** — `:graph`, `:search`, et toutes les vues que vos plugins enregistrent. Choisissez-en une, ou tapez-la : un deux-points ne commence jamais un nom de fichier, si bien que les étiquettes font aussi office d'adresse. `:graph` tapé à l'intérieur d'un dossier ouvre le graphe de ce dossier. Avec un plugin de page de démarrage installé, le séparateur du coffre ouvre cette page au premier clic et replie l'arborescence de fichiers au suivant.
- **Une ligne sur les volets qui ne contiennent aucun fichier** — un onglet vide affiche `vault / :blank`, le graphe `vault / :graph`, et le champ à côté est une barre d'adresse : tapez un chemin et <kbd>Entrée</kbd> l'ouvre dans ce volet, ou le crée. Les volets de la barre latérale conservent le titre propre d'Obsidian.
...
*En mode déplacer/renommer, ce même menu propose autre chose : le nom actuel de la note épinglé en haut pour la déplacer sans la renommer, les dossiers où la déplacer, et les noms déjà pris en rouge ; en choisir un demande quoi faire du fichier qui fait obstacle.*
...
- **Claude Opus 5 / 5.5** et **Claude Sonnet 5** (Anthropic, via Claude Code) — implémentation, traductions et documentation, sous la direction de l'auteur. Voir [divulgation de l'IA](#divulgation-de-lia).
