<!-- Traduzione di CHANGELOG.md — stato: commit 2739cf0.
...
- **Un nome già in uso chiede invece di rifiutare.** Spostare o rinominare su un nome già presente apre una finestra di dialogo con due percorsi modificabili: dove va il tuo file, e dove va il file di intralcio, rosso finché resta occupato. Ogni percorso è disegnato anche nel modo in cui lo disegna la barra del percorso, con le parti diverse colorate e accorciate per ultime. Entrambi i campi hanno un elenco; il secondo contiene le solite vie d'uscita — scambia i posti (va nella vecchia cartella del tuo file), scambia i nomi (resta dov'è e prende il vecchio nome del tuo file), scambia entrambi (prende il vecchio percorso del tuo file), `-1`, `-bak` e `-old` accanto al proprio nome, e i due nomi che avevano i file. Una via d'uscita il cui percorso è occupato appare in grigio. Sceglierne una riempie solo il campo; Applica sposta entrambi, collegamenti compresi, e Annulla non sposta nulla. Scegliere dall'elenco un nome già in uso chiede la stessa cosa, così come trascinare un file su una cartella che già contiene il suo nome.
...
- **La proposta è sempre ciò che Tab scriverebbe.** Dove i nomi smettono di concordare, il campo propone il passo verso il primo di essi, ed è la riga verso cui Tab si dirigerebbe a deciderlo; digitare sopra un nome lascia la sua estensione al suo posto e la propone davanti a essa; una cartella in cui si è appena entrati propone il suo primo passo. Prima esistevano stati in cui non veniva proposto nulla, e Tab scriveva comunque qualcosa. La sottolineatura del menu a discesa segue la proposta man mano che cambia, e Tab su una riga raggiunta con le frecce prende quella riga anziché quella accanto.
...
- **I nomi che iniziano con ciò che hai scritto vengono prima nel menu a discesa**, segnati da una linea sul bordo — blu dove condividono più di quanto hai scritto, verde sul ramo che la proposta imbocca dove si separano — davanti ai nomi che lo contengono soltanto. Ciascuno di essi sottolinea il passo che <kbd>Tab</kbd> compirebbe verso di lui, non solo quello proposto.
...
- **Backspace davanti a un'estensione rimasta da sola sale di una cartella**, come fa in un campo vuoto; l'estensione isolata scompare.
