...
- **Un nome già in uso chiede invece di rifiutare.** Spostare o rinominare su un nome già presente apre una finestra di dialogo che mostra entrambi i file con il loro percorso completo. Puoi rinominare quello di intralcio e proseguire, scambiare il posto con lui (va dove si trovava il tuo file, con il vecchio nome del tuo file), oppure scambiare i nomi con lui (resta dov'è e prende il vecchio nome del tuo file). Tra cartelle vengono offerte entrambe le opzioni; all'interno di una stessa cartella le due coincidono, quindi viene offerto solo lo scambio dei nomi. Annullare non sposta nulla. Ogni pulsante dice cosa farà non appena lo indichi. Scegliere dall'elenco un nome già in uso chiede la stessa cosa, così come trascinare un file su una cartella che già contiene il suo nome. Il nome nel campo di rinomina della finestra di dialogo è rosso finché è occupato.
