<!-- Tłumaczenie README.md — stan: commit 99fc83e.
...
- **Katalog główny skarbca pokazuje strony, jakie może otworzyć karta** — `:graph`, `:search` i wszystkie widoki, jakie rejestrują twoje wtyczki. Wybierz jedną albo ją wpisz: dwukropek nie zaczyna żadnej nazwy pliku, więc etykiety służą też za adres. `:graph` wpisane wewnątrz folderu otwiera graf tego folderu. Z zainstalowaną wtyczką strony startowej separator nazwy skarbca otwiera tę stronę przy pierwszym kliknięciu i chowa drzewo plików przy kolejnym.
...
*W trybie przenoszenia/zmiany nazwy ta sama lista oferuje co innego: na górze przypięta bieżąca nazwa notatki, aby przenieść ją bez zmiany nazwy; poniżej foldery, do których można ją przenieść; a nazwy już zajęte na czerwono; wybranie jednej z nich pyta, co zrobić z plikiem, który stoi na drodze.*
