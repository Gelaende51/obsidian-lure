<!-- Traducere a CHANGELOG.md — stare: commit 2739cf0.
...
- **Un nume deja ocupat întreabă în loc să refuze.** Mutarea sau redenumirea către un nume care există deja deschide un dialog cu două căi pe care le poți edita: unde ajunge fișierul tău, și unde ajunge fișierul care stă în cale, roșu cât timp încă e ocupat. Fiecare cale este desenată la fel cum o desenează bara de cale, cu părțile care diferă colorate și scurtate ultimele. Ambele câmpuri au o listă; a doua păstrează ieșirile obișnuite — schimbă locurile (ajunge în vechiul dosar al fișierului tău), schimbă numele (rămâne pe loc și preia vechiul nume al fișierului tău), schimbă amândouă (preia vechea cale a fișierului tău), `-1`, `-bak` și `-old` alături de numele propriu, și cele două nume pe care le-au avut fișierele. O ieșire a cărei cale e ocupată apare gri. Alegerea uneia doar completează câmpul; Aplică mută ambele, cu legături cu tot, iar Anulează nu mută nimic. Alegerea unui nume ocupat din listă întreabă același lucru, la fel și plasarea unui fișier peste un dosar care are deja acel nume.
...
- **Oferta este întotdeauna ce ar scrie <kbd>Tab</kbd>.** Acolo unde numele încetează să coincidă, câmpul oferă pasul spre primul dintre ele, iar rândul spre care s-ar îndrepta <kbd>Tab</kbd> decide care; scrierea peste un nume lasă extensia lui pe loc și e oferită înaintea ei; un dosar în care tocmai ai intrat oferă primul lui pas. Înainte existau stări în care nu se oferea nimic, iar <kbd>Tab</kbd> scria totuși ceva. Sublinierea din listă urmărește oferta pe măsură ce se schimbă, iar <kbd>Tab</kbd> pe un rând la care ai ajuns cu săgețile preia acel rând, nu pe cel de alături.
...
- **Numele care încep cu ce ai scris apar primele în listă**, marcate cu o linie pe margine — albastră unde au mai multe litere comune decât ai scris tu, verde pe ramura spre care duce oferta acolo unde se despart — înaintea numelor care doar îl conțin. Fiecare dintre ele subliniază pasul pe care l-ar face <kbd>Tab</kbd> spre el, nu doar cel oferit.
...
- **<kbd>Backspace</kbd> înainte de o extensie rămasă singură urcă un dosar**, ca într-un câmp gol; extensia singuratică dispare.
