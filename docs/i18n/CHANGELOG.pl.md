<!-- Tłumaczenie CHANGELOG.md — stan: commit 2739cf0.
...
- **Zajęta nazwa pyta zamiast odmawiać.** Przenoszenie lub zmiana nazwy na nazwę, która już tam jest, otwiera okno dialogowe z dwiema ścieżkami do edycji: dokąd trafia twój plik i dokąd trafia plik stojący na drodze, czerwoną, dopóki jest zajęta. Każda ścieżka jest też rysowana tak, jak rysuje ją pasek ścieżki, z częściami, które się różnią, pokolorowanymi i skracanymi na końcu. Oba pola mają listę; druga zawiera zwykłe sposoby wyjścia — zamień miejscami (trafia do starego folderu twojego pliku), zamień nazwy (zostaje i przejmuje starą nazwę twojego pliku), zamień oba (przejmuje starą ścieżkę twojego pliku), `-1`, `-bak` i `-old` obok własnej nazwy, oraz dwie nazwy, jakie mieli te pliki. Sposób wyjścia, którego ścieżka jest zajęta, jest wyszarzony. Wybranie jednego tylko wypełnia pole; Zastosuj przenosi oba, wraz z linkami, a Anuluj niczego nie przenosi. Wybranie zajętej nazwy z listy pyta o to samo, podobnie jak przeciągnięcie pliku na folder, który już ma tę nazwę.
...
- **Nazwy, które zaczynają się od tego, co wpisałeś, są na liście na pierwszym miejscu**, oznaczone linią wzdłuż krawędzi — niebieską tam, gdzie mają wspólnie więcej niż to, co wpisałeś, zieloną na gałęzi, którą podąża podpowiedź tam, gdzie się rozdzielają — przed nazwami, które to tylko zawierają. Każda z nich podkreśla krok, jaki wykonałby <kbd>Tab</kbd> w jej stronę, nie tylko ten, który jest podpowiadany.
