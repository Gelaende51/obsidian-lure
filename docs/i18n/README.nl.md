<!-- Vertaling van README.md — stand: commit 2cbb237.
...
- **Namen vullen zichzelf aan terwijl je typt** — wat <kbd>Tab</kbd> zou schrijven verschijnt achter de cursor, geselecteerd — de overeenkomst van de namen in de map, of de stap richting de eerste van hen; typen slikt het letter voor letter in, <kbd>→</kbd> neemt één letter, <kbd>Tab</kbd> of <kbd>End</kbd> neemt het heel, <kbd>Backspace</kbd> geeft het terug. De lijst blijft filteren op wat je typte, niet op wat werd aangeboden.
...
- **De hoofdmap van de kluis toont de pagina's die een deelvenster kan bevatten** — `:graph`, `:search`, en welke weergaven je plugins ook registreren. Kies er één, of typ hem: geen bestandsnaam begint met een dubbele punt, dus de labels dienen ook als adres. `:graph` getypt in een map opent de graaf van die map. Met een start-pagina-plugin geïnstalleerd opent het eigen scheidingsteken van de kluis die pagina bij de eerste klik en vouwt het de bestandsboom weg bij de volgende.
- **Een rij op deelvensters zonder bestand** — een leeg tabblad toont `vault / :blank`, de graaf `vault / :graph`, en het veld ernaast is een adresbalk: typ een pad en <kbd>Enter</kbd> opent het in dat deelvenster, of maakt het aan. Deelvensters in de zijbalk behouden Obsidians eigen titel.
