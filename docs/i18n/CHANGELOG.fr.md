...
- **Un nom déjà pris pose une question plutôt que de refuser.** Déplacer ou renommer vers un nom déjà présent ouvre une boîte de dialogue avec deux chemins modifiables : où va votre fichier, et où va le fichier qui fait obstacle, rouge tant qu'il reste pris. Chaque chemin est aussi dessiné comme la barre de chemin dessine le sien, avec les parties qui diffèrent colorées et raccourcies en dernier. Les deux champs ont une liste ; celle du second propose les issues habituelles — échanger les emplacements (il va vers l'ancien dossier de votre fichier), échanger les noms (il reste et prend l'ancien nom de votre fichier), échanger les deux (il prend l'ancien chemin de votre fichier), `-1`, `-bak` et `-old` accolés à son propre nom, ainsi que les deux noms qu'avaient les fichiers. Une issue dont le chemin est pris apparaît grisée. En choisir une ne fait que remplir le champ ; Appliquer déplace les deux, liens compris, et Annuler ne déplace rien. Choisir un nom déjà pris dans le menu déroulant pose la même question, tout comme déposer un fichier sur un dossier qui contient déjà son nom.
...
- **La proposition est toujours ce que Tab écrirait.** Là où les noms cessent de concorder, le champ propose le pas vers le premier d'entre eux, et c'est la ligne vers laquelle Tab se dirigerait qui en décide ; écrire par-dessus un nom laisse son extension en place et la propose devant elle ; un dossier dans lequel on vient d'entrer propose son premier pas. Auparavant, certains états ne proposaient rien, et Tab écrivait quand même quelque chose. Le soulignement du menu déroulant suit la proposition à mesure qu'elle change, et Tab sur une ligne que vous avez atteinte avec les flèches prend cette ligne plutôt que celle d'à côté.
...
- **Les noms qui commencent par ce que vous avez tapé passent en tête du menu déroulant**, marqués d'un trait sur leur bord — bleu là où ils partagent plus que ce que vous avez tapé, vert sur la branche que suit la proposition là où ils divergent — avant les noms qui ne font que le contenir. Chacun d'eux souligne le pas que <kbd>Tab</kbd> prendrait vers lui, pas seulement celui qui est proposé.
...
- **Retour arrière devant une extension restée seule remonte d'un dossier**, comme il le fait dans un champ vide ; l'extension isolée disparaît.
