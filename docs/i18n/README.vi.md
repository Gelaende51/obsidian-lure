...
- **Gõ một URL** — `https://`, `obsidian://`, hoặc một đường dẫn `file://` hay đường dẫn mã hóa phần trăm — và nó sẽ được mở chứ không bị coi là tên ghi chú. Địa chỉ web mở trong một tab của Web viewer (trình xem web) của chính Obsidian nếu bạn đã bật nó.
