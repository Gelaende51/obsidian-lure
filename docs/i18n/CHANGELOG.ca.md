...
- **Un nom ja agafat pregunta en lloc de refusar.** Moure o canviar el nom cap a un nom que ja hi és obre un diàleg amb dos camins que pots editar: on va el teu fitxer, i on va el fitxer que hi és al mig, en vermell mentre encara estigui agafat. Cada camí es dibuixa també tal com el dibuixa la barra de camí, amb les parts que difereixen acolorides i escurçades les últimes. Tots dos camps tenen una llista; la del segon conté les sortides habituals — intercanvia els llocs (va a l'antiga carpeta del teu fitxer), intercanvia els noms (es queda i pren l'antic nom del teu fitxer), intercanvia-ho tot (pren l'antic camí del teu fitxer), `-1`, `-bak` i `-old` al costat del seu propi nom, i els dos noms que tenien els fitxers. Una sortida el camí de la qual ja estigui agafat es mostra en gris. Triar-ne una només omple el camp; Aplica mou els dos, enllaços inclosos, i Cancel·la no mou res. Triar un nom ja agafat des del desplegable pregunta el mateix, i també arrossegar un fitxer cap a una carpeta que ja té aquell nom.
...
- **L'oferta és sempre el que escriuria <kbd>Tab</kbd>.** On els noms deixen de coincidir, el camp ofereix el pas cap al primer d'ells, i la fila cap on aniria <kbd>Tab</kbd> ho decideix; escriure sobre un nom deixa dreta la seva extensió i s'ofereix davant seu; una carpeta a la qual just has entrat ofereix el seu primer pas. Abans, hi havia estats en què no s'oferia res i el <kbd>Tab</kbd> escrivia alguna cosa igualment. El subratllat del desplegable segueix l'oferta a mesura que canvia, i <kbd>Tab</kbd> en una fila a la qual has arribat amb les fletxes pren aquesta fila i no la del costat.
...
- **Els noms que comencen pel que has escrit van primer al desplegable**, marcats amb una línia a la vora — blava on comparteixen més del que has escrit, verda a la branca cap on va l'oferta on se separen — per davant dels noms que només el contenen. Cadascun subratlla el pas que <kbd>Tab</kbd> hi faria, no només el que s'ofereix.
...
- **<kbd>Retrocés</kbd> abans d'una extensió que ha quedat sola puja una carpeta**, igual que fa en un camp buit; l'extensió sola desapareix.
