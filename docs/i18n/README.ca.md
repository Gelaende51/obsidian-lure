<!-- Traducció de README.md — estat: commit 99fc83e.
...
- **L'arrel del cofre llista les pàgines que un panell pot contenir** — `:graph`, `:search` i les vistes que registrin els teus connectors. Tria'n una, o escriu-la: els dos punts no comencen cap nom de fitxer, així que les etiquetes fan també d'adreça. `:graph` escrit dins d'una carpeta obre el graf d'aquella carpeta. Amb un connector de pàgina d'inici instal·lat, el delimitador propi del cofre obre aquella pàgina al primer clic i plega l'arbre de fitxers al següent.
...
*En mode moure/reanomenar el mateix desplegable ofereix una altra cosa: el nom actual de la nota fixat a dalt per moure-la sense reanomenar-la, carpetes on portar-la, i els noms ja ocupats en vermell; triar-ne un pregunta què cal fer amb el fitxer que hi ha al davant.*
