...
- **사용량** — 2026년 8월 3일–9월 6일, 22 세션, 약 13,378회 응답: 생성 토큰 약 1,630만, 전송 약 6,230만, 캐시 재읽기 약 42억 4,510만(총 약 43억 2,360만).
...
- **어느 항목이든 오른쪽 클릭하거나 끌 수 있습니다** — 파일 탐색기 자체의 컨텍스트 메뉴가 항목 하나하나까지 그대로 나오고, 끌기 동작도 같습니다. 보관함 밖의 경로에는 그에 맞춰 만든 같은 구성의 메뉴가 붙으며, 시스템 휴지통을 거치는 *Delete*(삭제)까지 갖췄습니다.
...
- **URL을 입력하면** — `https://`, `obsidian://`, 또는 `file://`이나 퍼센트 인코딩된 경로 — 노트 이름으로 취급하는 대신 열립니다. 웹 주소는 Obsidian 자체의 Web viewer(웹 뷰어)를 켜 두었다면 그 탭에서 열립니다.
...
- **<kbd>F2</kbd>** 키는 인라인 제목과 경로 표시줄 사이를 오갑니다. 확장자를 뺀 이름에서 시작해, 더 누를수록 전체 경로까지 넓어집니다. 제목이 스크롤되어 화면에서 벗어나 있어도 Obsidian의 이름 바꾸기 대화 상자를 깔끔하게 통과합니다. 주소 표시줄 같은 동작을 원한다면 *경로 표시줄에 포커스* 명령에 키를 지정할 수 있습니다.
