...
- **목록은 지금 서 있는 항목에서 열리며**, 화살표 키나 포인터로 훑으면 가리키는 것이 입력란에 채워집니다. 목록의 양쪽 끝을 벗어나면 입력해 두었던 것이 돌아오고, 포인터를 목록 밖으로 빼면 강조 표시가 원래 있던 자리로 돌아갑니다.
- **연필 폴더 버튼**은 같은 조작을 이동/이름 바꾸기로 전환하며, 검사는 Obsidian이 하는 방식 그대로입니다.
...
- **이름은 입력하는 동안 스스로 채워집니다** — 폴더 안의 이름들이 일치하는 곳에서는 일치하는 부분이 커서 뒤에 선택된 채로 나타납니다. 계속 입력하면 한 글자씩 받아들여지고, <kbd>Tab</kbd>이나 <kbd>→</kbd>는 통째로 가져가며, <kbd>Backspace</kbd>는 되돌립니다. 목록은 제안된 내용이 아니라 입력한 내용으로 계속 걸러집니다.
...
- **보관함 루트에는 창이 담을 수 있는 페이지가 나열됩니다** — `:graph`, `:search`, 그리고 플러그인이 등록하는 보기. 하나를 고르거나 직접 입력하세요. 콜론으로 시작하는 파일 이름은 없으므로 레이블이 주소 역할도 합니다. 시작 페이지 플러그인이 설치되어 있으면 보관함 자체의 구분자가 첫 클릭에 그 페이지를 열고, 다음 클릭에 파일 트리를 접습니다.
