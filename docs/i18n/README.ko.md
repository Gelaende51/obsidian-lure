...
- **사용량** — 2026년 8월 3–22일, 22 세션, 약 11,820회 응답: 생성 토큰 약 1,540만, 전송 약 5,550만, 캐시 재읽기 약 36억 4,420만(총 약 37억 1,500만).
...
- **목록은 지금 서 있는 항목에서 열리며**, 화살표나 포인터로 훑으면 가리키는 것이 입력란에 채워집니다. 양쪽 끝을 지나면 입력해 두었던 것을 돌려줍니다.
- **이름은 입력하는 동안 스스로 채워집니다** — 폴더 안의 이름들이 일치하는 데까지, 일치하는 부분이 커서 뒤에 선택된 채로 나타납니다. <kbd>Tab</kbd>이나 <kbd>→</kbd>가 통째로 가져가고, <kbd>Backspace</kbd>가 되돌립니다.
- **<kbd>Tab</kbd>은 셸처럼 완성합니다**: 이름들이 일치하는 데까지 입력을 늘리고, 남은 이름이 하나가 되는 순간 그 폴더로 들어갑니다. 경로 끝을 지나면 대신 선택 범위를 넓힙니다 — 이름, 확장자를 포함한 이름, 보관함부터의 경로, 시스템 루트부터의 경로. <kbd>Shift</kbd>+<kbd>Tab</kbd>은 같은 길을 거꾸로 갑니다.
- **오른쪽 클릭으로 복사** — 두 번이면 이름, 세 번이면 그 오른쪽 전부, 빈 공간에서는 전체 경로나 시스템 경로.
- **노트를 행의 폴더 위로 끌어다 놓으면** 링크까지 그대로 그곳으로 옮겨집니다. 보관함 이름도 받아 주어 루트로 옮길 수 있습니다. 여러 개를 고르면 하나처럼 함께 옮겨지고, 받을 수 없는 폴더는 아무것도 보여 주지 않습니다.
- **URL을 입력하면** — `https://`, `obsidian://`, 또는 `file://`이나 퍼센트 인코딩된 경로 — 노트 이름으로 읽는 대신 열립니다.
- **긴 경로는 글자가 남아도는 곳부터 줄어듭니다** — 옆 폴더와 구별되는 선을 넘는 일은 없고 — 더 줄일 것이 없을 때에야 스크롤합니다. 줄어든 이름을 가리키면 전체가 다시 보입니다.
...
- **설정:** 정렬, 구분 기호 프리셋, 어느 클릭이 목록을 여는지, 보관함 이름, 숨김 파일, 파일 확장자.
