...
- **출처** — Obsidian 플러그인을 쓰는 모델은 사람들이 써서 거저 내어준 오픈 소스 코드와 문서, 포럼 답변, 버그 리포트에서 그것을 배웠습니다. 그중 누구도 요청받거나, 이름이 오르거나, 대가를 받지 않았습니다. 여기서 가장 큰, 이름 없는 기여이며 이 플러그인보다 여러분의 지원을 받을 자격이 있습니다. 어딘가에 보낼 곳을 고른다면 그쪽으로 보내십시오.
