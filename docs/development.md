...
Both need Obsidian running with `--remote-debugging-port=9222` (add it to `~/.config/obsidian/user-flags.conf`) and the demo vault open. That vault now lives at `../obsidian-plugin-template/.personal/vault` — outside this repository and outside git, so nothing about it reaches a commit. Obsidian names a vault after its directory, so it appears as **vault**; that is the name the `SCENES` table and `OBSIDIAN_VAULT` use. It needs this plugin symlinked into its `.obsidian/plugins/lure/`, and, for the compatibility suite, the peer plugins installed; the suite skips any that aren't. With several vaults open, choose the window with `OBSIDIAN_VAULT=<name>`. **Comment the port back out when you're done** — a shipped build must never be developed against an open debugging port.
