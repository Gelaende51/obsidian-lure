...
- **The dropdown opens on the entry you're standing in**, and arrowing or hovering through it fills the field with what you're pointing at. A row you point at is shown as the offer it would make; moving off either end of the list gives back what you had typed, and taking the pointer off it hands the highlight back to where you were. The list follows the caret: the folder it is in, filtered by the letters in front of it.
- **Pencil Folder button** switches the same interactions to move/rename, validated as Obsidian validates. A name already taken is red in the list, and choosing it opens both paths side by side to edit, with swapping places, names or both a pick away.
...
- **Names complete themselves as you type** — what <kbd>Tab</kbd> would write appears after the caret, selected and spelled the way the name is, whatever case you typed — the agreement of the folder's names, or the step toward the first of them; typing swallows it letter by letter, <kbd>→</kbd> takes one letter, <kbd>Tab</kbd> or <kbd>End</kbd> takes it whole, <kbd>Backspace</kbd> takes it back. The dropdown keeps filtering by what you typed, not by what was offered.
- **<kbd>Tab</kbd> completes like a shell**: it extends what you typed as far as the names in that folder agree, walks toward one of them a step at a time when they don't, and steps into a folder only once one name is left. Past the end of the path it widens the selection instead: name, name with extension, path from the vault, path from the system root. <kbd>Shift</kbd>+<kbd>Tab</kbd> walks the same road backwards — marking what it gives back rather than deleting it — and past the start of it keeps going up the path, then loops round to the system path. Either way round, a lap comes back to the path you built.
...
- **Drop text onto the row to write it down** — onto a folder or the vault name to name a new note for it, onto the note's own name to add it to the end of what you are reading. A file off your desktop works the same way, and the row rings blue while it would land.
- **The field wears the colour of what it names** — the same colour its row has in the dropdown — and **goes red** once nothing answers to it, so you can see before pressing <kbd>Enter</kbd> whether it will open a note or make one.
- **HTML files show as pages**, in a frame with every permission withheld — no scripts, no network, no origin of its own — with the stylesheets and images beside the file carried in so a saved page still looks like itself. The source is one press away.
...
- **<kbd>F2</kbd>** alternates between the inline title and the path bar, opening on the name without its extension and walking out to the full paths on further presses. It passes cleanly through Obsidian's rename dialog when the title is scrolled out of view. A *Focus the path bar* command walks the same rungs without renaming; the settings' *Hotkeys* row takes you to bind it.
...
- **The vault root lists the pages a pane can hold** — `:graph`, `:search`, and whatever views your plugins register. Pick one, or type it: a colon begins no filename, so the labels double as an address. `:graph` typed inside a folder opens the graph of that folder. With a start-page plugin installed, the vault's own delimiter opens that page on the first click and folds the file tree on the next.
- **A row on panes that hold no file** — an empty tab reads `vault / :blank`, the graph `vault / :graph`, and the field beside it is an address bar: type a path and <kbd>Enter</kbd> opens it in that pane, or makes it. Sidebar panes keep Obsidian's own title.
...
*In move/rename mode the same dropdown changes what it offers: the note's current name pinned at the top to move it without renaming, folders to move it into, and names already taken in red; picking one asks what to do about the file in the way.*
...
- [Folder notes](obsidian://show-plugin?id=folder-notes) — the delimiter opens a folder's note instead of revealing the folder, making every segment of the path somewhere you can go, however deep: the note is resolved from that plugin's own convention rather than left to it to answer. It is also the only one that publishes such a convention; [Folder Note](obsidian://show-plugin?id=folder-note-plugin) and [create folder notes with dropdown](obsidian://show-plugin?id=create-folder-notes-with-dropdown) publish none and never claim the header path, so with those the delimiter reveals the folder as usual.
