...
- **Usage** — 3 Aug – 17 Sep 2026, 23 sessions, \~14,844 responses: \~19.2 M tokens generated, \~85.2 M sent, \~4800.6 M cached re-reads (\~4905.0 M total).
...
- **Names complete themselves as you type** — where the folder's names agree, the agreement appears after the caret, selected; typing swallows it letter by letter, <kbd>Tab</kbd> or <kbd>→</kbd> takes it whole, <kbd>Backspace</kbd> takes it back. The dropdown keeps filtering by what you typed, not by what was offered.
...
- **The field wears the colour of what it names** — the same colour its row has in the dropdown, grey for a folder's note — and **goes red** once nothing answers to it, so you can see before pressing <kbd>Enter</kbd> whether it will open a note or make one.
...
- **The vault root lists the pages a pane can hold** — `:graph`, `:search`, and whatever views your plugins register. Pick one, or type it: a colon begins no filename, so the labels double as an address. With a start-page plugin installed, the vault's own delimiter opens that page on the first click and folds the file tree on the next.
